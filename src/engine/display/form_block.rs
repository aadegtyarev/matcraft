...
mod tests {
    ...
    fn test_form_block_matches_operator_mockup() {
        // The #28 cluster form осснуть: о-/об- + сса- + -ну- + -ть, Possible.
        let rd = root_data("сса").unwrap();
        let vf = find_form("сса", "о-/об-", "ну", "ть");
        assert_eq!(vf.form, "осснуть", "sanity: engine builds осснуть");
        let block = format_form_block(rd, &vf);

        assert!(block.contains("осснуть  ·  possible"), "header: {block}");
        assert!(
    ...
    #[test]
    fn test_form_block_28_cluster_is_self_explanatory() {
        // #28: the ugly cluster вссснёт is built by the engine; the block must
        // mark it possible + explain non-attestation, making it self-explanatory
        // (rich-output does NOT fix the morphonology — that stays #28).
        let rd = root_data("сса").unwrap();
        let vf = find_form("сса", "вз-/вс-", "ну", "нёт");
        assert_eq!(vf.form, "вссснёт", "sanity: engine builds the #28 cluster");
        let block = format_form_block(rd, &vf);
        assert!(block.contains("вссснёт  ·  possible"), "{block}");
        assert!(
